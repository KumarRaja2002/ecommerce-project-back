import { db } from "./lib/db"; // Import the db connection from dbConnection.ts
import { products } from "./schemas/product"; // Ensure you have the schema file set up correctly
...
const data = [
  ...
  export class seedProducts {
    public async insertProducts(c: Context) {
      try {
        // Check the database connection before proceeding
        console.log("Checking database connection...");
        const isConnected = await db.select().from(products).limit(1); // Example check to ensure connection works
        if (!isConnected) {
          return c.json({ message: 'Database connection failed' }, 500);
        }
  
        console.log("Inserting data into the database...");
        await db.insert(products).values(data); // This is where insertion happens
